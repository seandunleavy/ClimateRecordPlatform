...
8. **Refresh (v2.1)** — scheduled re-pull of the locked cohort (`run_refresh.py`): force-download → size change detection → silver/QC for changed stations → optional full gold + dbt + web export; **observation_diff** counts new daily keys vs value corrections (run meta, not extra fact columns)  
